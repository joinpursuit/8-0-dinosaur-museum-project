...
for (let item of purchases){
  let cost = calculateTicketPrice(ticketData, item); if (typeof cost === "string"){
  return cost;
}
total += cost;
...
if (item.extras.length != 0){
  receipt += `${First(item.entrantType)} ${ticketData[item.ticketType].description}: $${(cost / 100).toFixed(2)} (${item.extras.join(", ")})\n`;
} else {
  receipt += `${First(item.entrantType)} ${
    ticketData[item.ticketType].description}: $${(cost / 100).toFixed(2)}\n`;
      }
    }
