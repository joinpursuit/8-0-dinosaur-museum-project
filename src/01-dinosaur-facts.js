...
let mya
  ...
  for(oneDino of dinosaurs){
    if(id === oneDino.dinosaurId){
        if(oneDino.mya.length === 2) {
        mya = oneDino.mya[1]
        }else{
        mya = oneDino.mya[0]
        }

        return oneDino.name + " (" + oneDino.pronunciation + ")\n" + oneDino.info + " It lived in the " + oneDino.period + " period, over " + mya + " million years ago."
        }
    }
    if(id === "incorrect-id"){
