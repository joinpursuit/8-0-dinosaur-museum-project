...
function getLongestDinosaur(dinosaurs) {
  ...
  dinosaurs.forEach((element) => {
    if (element.lengthInMeters > length) {
      length = element.lengthInMeters;
      largestDino = element;
    }
  });
  finalDino[largestDino.name] = largestDino.lengthInMeters * 3.281;
  return finalDino;
}
  ...
  for (let i = 0; i < dinosaurs.length; i++) {
    if (dinosaurs[i].mya.length === 2) {
      if (dinosaurs[i].mya[0] >= mya && dinosaurs[i].mya[1] <= mya) {
        if (dinosaurs[i][key]) {
          sameDinoEra.push(dinosaurs[i][key]);
        } else {
          sameDinoEra.push(dinosaurs[i].dinosaurId);
        }
      }
