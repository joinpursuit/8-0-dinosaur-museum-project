...
function getTallestDinosaur(dinosaurs) {
 let tallest = {}
 let tallestNow = dinosaurs[0];
  for (let i = 1; i < dinosaurs.length; i++){
    if (dinosaurs[i].lengthInMeters > tallestNow.lengthInMeters ){
      tallestNow = dinosaurs[i]
    }
  }
    if (tallestNow){
          tallest[tallestNow.name] = tallestNow.lengthInMeters*3.281
    }
return tallest
}
...
function getDinosaurDescription(dinosaurs, id) {
  ...
  for (let dino of dinosaurs){ //this means let each(dino being each dinosaurs[i] element) element of dinosaurs
    if (dino.dinosaurId === id){//  dinosaurs[i].dinosaurId
 ...
 * ---------------------
 ⁡⁢⁣⁣* Returns an array of dinosaurs who were alive at the given `mya` (i.e. "millions of years ago") value. ⁡⁣⁣⁢⁡⁣⁣⁢LOOKING TO RETURN AN ARRAY WITH THE "MYA KEY" INFO
