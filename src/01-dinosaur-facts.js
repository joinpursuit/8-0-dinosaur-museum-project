...
function getTallestDinosaur(dinosaurs) {
  ...
  let obj = {};
  let tallestInFeet = dinosaurs[0].lengthInMeters * 3.281;
  let dinoName = dinosaurs[0].name;
  for (let i = 1; i < dinosaurs.length; i++){
    
 let heightInFeet = dinosaurs[i].lengthInMeters * 3.281;
 

  if (tallestInFeet < heightInFeet){
    // console.log(`${dinoName} ${tallestInFeet}`);
    tallestInFeet = heightInFeet;
    dinoName = dinosaurs[i].name;
  // console.log(`newest height found: ${dinoName} ${tallestInFeet}`)
   obj = {Brachiosaurus: tallestInFeet};
  // console.log(obj);
} 
}
// return `${dinoName}: ${tallestInFeet}`;
return obj; 
}
 ...
 * and a bony frill with elaborate ornamentation of projections, knobs, and spikes. 
...
if(dinosaurs[i].dinosaurId === id && dinosaurs[i].mya[dinosaurs[i].mya.length - 1] > 80){
      description = `${dinosaurs[i].name} (${dinosaurs[i].pronunciation})\n${dinosaurs[i].info} It lived in the Late Cretaceous period, over ${dinosaurs[i].mya[dinosaurs[i].mya.length - 1]} million years ago.`;
      } else if(dinosaurs[i].dinosaurId === id && dinosaurs[i].mya[dinosaurs[i].mya.length - 1] < 80){
        description = `${dinosaurs[i].name} (${dinosaurs[i].pronunciation})\n${dinosaurs[i].info} It lived in the Early Cretaceous period, over ${dinosaurs[i].mya[dinosaurs[i].mya.length - 1]} million years ago.`;
    }
  }
return description;
}
 ...
 * allows for the `mya` value to be equal to the given value or one less. 
    ...
    if(!key){
      if(dinosaur.mya.length > 1){
        if(mya < dinosaur.mya[0] && mya >dinosaur.mya[1]){
          myaList.push(dinosaur.dinosaurId);
        }
      }
