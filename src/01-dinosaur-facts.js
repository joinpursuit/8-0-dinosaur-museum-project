/*
  ...
  you may use the `exampleDinosaurData` variable 
  below to gain access to tickets data. 
  This data is pulled from the `data/dinosaurs.js` file.

  You may use this data to test your functions. 
  You may assume the shape of the data remains the 
  same but that the values may change.

  Keep in mind that your functions must still 
  have and use a parameter for accepting all dinosaurs.
*/
 ...
 *  //> { Brachiosaurus: 98.43 }
 */
function getTallestDinosaur(dinosaur) {
  if (!dinosaur.length) {
    return{};
  }
  let tallDino = dinosaur[0];
    for (let i =  1; i < dinosaur.length; i++) {
      let dinoHeight = dinosaur[i];
     if (dinoHeight.lengthInMeters > tallDino.lengthInMeters) {
    tallDino = dinoHeight;
    }
  }
  let ftInLen = tallDino.lengthInMeters * 3.281;
  return {[tallDino.name]: ftInLen};
}
 ...
 *
 * NOTE: Carefully view the test output and example below to 
 * see how the returned string should be formatted.
 *
 ...
 *  //> "Xenoceratops (ZEE-no-SEH-ruh-tops)\nXenoceratops had horns 
 * and a bony frill with elaborate ornamentation of projections, knobs, 
 * and spikes. It lived in the Early Cretaceous period, 
...
function getDinosaurDescription(dinosaur, id) {
  let dinoDescript = `A dinosaur with an ID of '${id}' cannot be found.`;
  let dino = '';
   for (let i = 0; i < dinosaur.length; i++) {
      dino = dinosaur[i];
      if (dino.dinosaurId === id) {
        dinoDescript = `${dino.name} (${dino.pronunciation})\n${dino.info} It lived in the ${dino.period} period, over ${dino.mya[dino.mya.length -1]} million years ago.`
       }
      }
   return dinoDescript;
}
 ...
 * The array should only include data of dinosaurs who lived during 
...
function getDinosaursAliveMya(dinosaurs, mya, key) {
  ...
  for (let i = 0; i < dinosaurs.length; i++) {
    deSaurs = dinosaurs[i];
    if (deSaurs.mya.length === 1) {
      if (deSaurs.mya[0] === mya || deSaurs.mya[0] - 1 === mya) {
        if(deSaurs[key]) {
          arr.push(deSaurs[key]);
        } else {
          arr.push(deSaurs.dinosaurId);
        }
      }
    } else {
      if(deSaurs.mya[0] >= mya && deSaurs.mya[1] <= mya) {
        if(deSaurs[key]) {
          arr.push(deSaurs[key]);
        } else {
          arr.push(deSaurs.dinosaurId);
